...
function ReplTable({ rows, empty }: { rows: ReplicationRow[]; empty: string }) {
  ...
  return (
    <table className="w-full text-sm">
      ...
      <tbody>
        {rows.map((r) => (
          <tr key={`${r.order_number}-${r.operation_date}`} className="border-t border-slate-100">
            ...
            <td className="px-3 py-2 font-mono text-slate-500">{r.remote_id ?? "—"}</td>
            <td className="px-3 py-2 text-red-600 text-xs">{r.error ?? ""}</td>
            <td className="px-3 py-2 text-slate-400 text-xs">{fmtTime(r.updated_at)}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
export default function Dashboard() {
  ...
  const intervalMs = getConfig().REPLICATION_INTERVAL_MS;

  const fresh =
    worker.last_cycle_at !== null && Date.now() - worker.last_cycle_at < intervalMs * 2;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Card title="Worker" sub={`last cycle ${fmtTime(worker.last_cycle_at)}`}>
          {fresh ? (
            ...
            <span className="inline-flex items-center gap-1 text-red-700">
              <span className="w-2 h-2 rounded-full bg-red-500" />stopped / stale
            </span>
          )}
        </Card>
        <Card title="Replicated" sub="success + noop">{c.totalReplicated}</Card>
        <Card title="Failures">{c.failures}</Card>
        <Card title="DB size">{dbKb} KB</Card>
      </div>

      {worker.last_error ? (
        <div className="mb-4 text-sm text-red-700">Last cycle error: {worker.last_error}</div>
      ) : null}

      <section className="mb-6">
        <h2 className="font-semibold mb-2">Active replications ({active.length})</h2>
